...
{Fore.CYAN}{'=' * 60}
    ...
    def view_system_stats(self):
        ...
        if collab_stats:
            print(f"{Fore.WHITE}   Total Co-authorships: {collab_stats.get('total_coauthorships', 'N/A')}")
            print(f"   Total Supervisions: {collab_stats.get('total_supervisions', 'N/A')}")
            print(f"   Total Teamworks: {collab_stats.get('total_teamworks', 'N/A')}")
    ...
    def view_all_researchers(self):
        ...
        for i, researcher in enumerate(top_researchers, 1):
            table_data.append([
                ...
                researcher.get('publications_count', 0),
                researcher.get('projects_count', 0),
                researcher.get('total_score', 0)
            ])
    ...
    def view_most_collaborative_pairs(self):
        ...
        print(f"\n{Fore.CYAN}{'=' * 80}")
        print(f"{Fore.YELLOW}MOST COLLABORATIVE PAIRS (Top {min(limit, len(pairs))})")
        ...
        for i, pair in enumerate(pairs[:limit], 1):
            table_data.append([
                ...
                pair.get('relationship_type', 'N/A')
            ])

        headers = ['#', 'Researcher 1', 'Researcher 2', 'Count', 'Type']
        print(tabulate(table_data, headers=headers, tablefmt='simple_grid'))
    ...
    def view_network_statistics(self):
        ...
        print(f"{Fore.WHITE}   Co-authorships: {stats.get('total_coauthorships', 'N/A')}")
        print(f"   Supervisions: {stats.get('total_supervisions', 'N/A')}")
        print(f"   Teamworks: {stats.get('total_teamworks', 'N/A')}")
